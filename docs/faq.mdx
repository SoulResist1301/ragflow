...
RAGFlow supports indexing documents from local folders that are already mounted in the Docker container. This allows you to index files through RAGFlow's interface without manually uploading them, while still storing them in RAGFlow's storage system for processing.
