...
### How to index local folders already mounted in Docker?

RAGFlow supports zero-copy indexing from local folders mounted in the Docker container. Files **remain in the mounted folder** and are referenced directly - only the parsed index is stored in RAGFlow's database.

**To mount and index local folders:**

1. Add a volume mount in `docker/docker-compose.yml`:
   ```yaml
   volumes:
     - /path/to/your/documents:/ragflow/mounted_data:ro
   ```
   The `:ro` (read-only) flag is recommended for safety. **Keep this mount active** - RAGFlow needs ongoing access to the files.
...
3. In the web interface, navigate to your dataset and click **Add file** > **Index local folder**.

4. Enter the path inside the container (e.g., `/ragflow/mounted_data/my-documents`) and click **OK**.

Files will be indexed in place without copying. The mounted folder must remain accessible for RAGFlow to parse and retrieve documents.

For detailed instructions, see [Index local folders](./guides/index_local_folder.md).
