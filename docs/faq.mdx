...
RAGFlow supports indexing documents from local folders that are already mounted in the Docker container. Files are **read from** the mounted folder (which can be read-only) and **copied to** RAGFlow's internal storage (MinIO/S3) for processing.
...
1. Add a volume mount in `docker/docker-compose.yml`:
   ...
   ```
   The `:ro` (read-only) flag is safe because files are only read from this location, not written to it.
...
Files will be copied from the mounted folder into RAGFlow's storage system. Your original files remain untouched.
